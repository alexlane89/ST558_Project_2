...
> Create a geom map that identifies the location and number of prizes in each country per category.
...
wm <- ggplot() + 
  ...
  geom_point(data = z$Birth_Country_Now,
             aes(z$Birth_Long, z$Birth_Lat)
             )

#wm <- sf::st_read(system.file("shape."))
...
```{r}
gen_table <- table(z$Birth_Country_Now, z$gender)
...
> Could we evaluate if the overall number of recipients has increased significantly?

```{r}
num_tot <- z |>
  group_by(awardYear) |>
  count(name = "Number of Recipients")
