...
```{r}
s <- ggplot(z_2, aes(awardYear, n, fill = factor(gender, levels = c("male", "female"))))
s + geom_col()
```


> Create a geom map that identifies the location and number of prizes in each country per category. Based on the ggplot2 help graphic / cheat sheet, the "sf" or "simple features" package is required to intereact with geometric data.
